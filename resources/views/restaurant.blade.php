@extends('layouts.welcome')
@section('meta')
    <meta name="description" content="Welcome to our hotel, where comfort meets luxury. Book your stay with us today!">
    <meta name="keywords" content="hotel, luxury, comfort, booking">
    <meta name="author" content="Your Name">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

@section('title', 'Restaurants')

@section('content')
    <div class="section big-55-height over-hide z-bigger">

        <div id="poster_background-res"></div>
        <div id="video-wrap" class="parallax-top">
            <video id="video_background" preload="auto" autoplay loop="loop" muted="muted" poster="img/trans.png">
                <source src="video/video-res.mp4" type="video/mp4">
            </video>
        </div>
        <div class="dark-over-pages"></div>

        <div class="hero-center-section pages">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-12 parallax-fade-top">
                        <div class="hero-text">Restaurant</div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="section padding-top over-hide">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-md-4">
                    <div class="services-box restaurant text-center">
                        <img src="img/res-1.png" alt="">
                        <h5 class="mt-3">High quality foods</h5>
                        <p class="mt-3">We offer an ample selection of the highest quality foods and beverages, always
                            varied and full of imagination. Excellent quality products prepared carefully</p>
                        <a class="mt-1 btn btn-primary" href="restaurant">read more</a>
                    </div>
                </div>
                <div class="col-md-4 mt-5 mt-md-0">
                    <div class="services-box restaurant text-center">
                        <img src="img/res-2.png" alt="">
                        <h5 class="mt-3">Inspiring recipes</h5>
                        <p class="mt-3">
                            Embark on a flavourful adventure with our tailored recipe collections that bring the art of
                            cooking to life. Come and taste delicious recipes to make you feel at home.</p>
                        <a class="mt-1 btn btn-primary" href="restaurant">read more</a>
                    </div>
                </div>
                <div class="col-md-4 mt-5 mt-md-0">
                    <div class="services-box restaurant text-center">
                        <img src="img/res-3.png" alt="">
                        <h5 class="mt-3">Salutary meals</h5>
                        <p class="mt-3">Salutary meals. Good taste, it's in our culture. main dishes. Our menu. get in
                            touch. Contact us. available: 24hr. Call us at(234) 813-301-1117.
                        </p>
                        <a class="mt-1 btn btn-primary" href="restaurant">read more</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="section padding-top-bottom z-bigger">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-md-8 align-self-center">
                    <div class="subtitle with-line text-center mb-4">main dishes</div>
                    <h3 class="text-center padding-bottom-small">Our Menu</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-6" data-scroll-reveal="enter bottom move 50px over 0.7s after 0.2s">
                    <div class="restaurant-box">
                        <img src="images/fired-rice.jpg" alt="">
                        <h6><span>Jollof Rice</span></h6>
                        <p><span>Rice / Tomatoes / Spices</span></p>
                        <h5><span>₦3,000</span></h5>
                    </div>
                </div>
                <div class="col-md-6 mt-4 mt-md-0" data-scroll-reveal="enter bottom move 50px over 0.7s after 0.4s">
                    <div class="restaurant-box">
                        <img src="images/egg-sauce.jpg" alt="">
                        <h6><span>Chips & Egg Sauce</span></h6>
                        <p><span>Potatoes / Eggs / Sauce</span></p>
                        <h5><span>₦5,000</span></h5>
                    </div>
                </div>
                <div class="col-md-6 mt-4" data-scroll-reveal="enter bottom move 50px over 0.7s after 0.2s">
                    <div class="restaurant-box">
                        <img src="images/fried-chicken.jpg" alt="">
                        <h6><span>Chicken</span></h6>
                        <p><span>Chicken / Spices / Oil</span></p>
                        <h5><span>₦4,000</span></h5>
                    </div>
                </div>
                <div class="col-md-6 mt-4" data-scroll-reveal="enter bottom move 50px over 0.7s after 0.4s">
                    <div class="restaurant-box">
                        <img src="images/pounded-yam.jpg" alt="">
                        <h6><span>Pounded Yam</span></h6>
                        <p><span>Yam / Soft / Well Packaged</span></p>
                        <h5><span>₦3,500</span></h5>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="section padding-top-bottom over-hide background-grey">
        <div class="container">
            <div class="row justify-content-center padding-bottom-smaller">
                <div class="col-md-8">
                    <div class="subtitle with-line text-center mb-4">Place your order Now</div>
                    <h3 class="text-center padding-bottom-small">Protein</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-12 col-lg-12">
                    <table id="protein-table" style="width: 100%;" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Menu</th>
                                <th>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1.</td>
                                <td>Beef</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>2.</td>
                                <td>Chicken</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>3.</td>
                                <td>Chicken Big</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>4.</td>
                                <td>Chicken Extra Big</td>
                                <td>₦3,500</td>
                            </tr>
                            <tr>
                                <td>5.</td>
                                <td>Cow Tail</td>
                                <td>₦6,000</td>
                            </tr>
                            <tr>
                                <td>6.</td>
                                <td>Cow Head</td>
                                <td>₦5,000</td>
                            </tr>
                            <tr>
                                <td>7.</td>
                                <td>Fish</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>8.</td>
                                <td>Small Fish</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>9.</td>
                                <td>Goat Meat</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>10.</td>
                                <td>Goat Meat B/S</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>11.</td>
                                <td>Kayanciki</td>
                                <td>₦3,000</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

    <div class="section padding-top z-bigger">
        <div class="container">
            <div class="row justify-content-center padding-bottom-smaller">
                <div class="col-md-8">
                    <div class="subtitle with-line text-center mb-4">Place your order Now</div>
                    <h3 class="text-center padding-bottom-small">Soup</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-12 col-lg-12">
                    <table id="soup-table" style="width: 100%;" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Menu</th>
                                <th>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1.</td>
                                <td>Bitterleaf Soup</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>2.</td>
                                <td>Draw Soup</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>3.</td>
                                <td>Egusi Soup</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>4.</td>
                                <td>Vegetable Soup</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>5.</td>
                                <td>Vegetable Okro Soup</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>6.</td>
                                <td>Cream Soup</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>7.</td>
                                <td>Stew</td>
                                <td>₦1,000</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

    <div class="section padding-top-bottom over-hide background-grey">
        <div class="container">
            <div class="row justify-content-center padding-bottom-smaller">
                <div class="col-md-8">
                    <div class="subtitle with-line text-center mb-4">Place your order Now</div>
                    <h3 class="text-center padding-bottom-small">Swallow</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-12 col-lg-12">
                    <table id="swallow-table" style="width: 100%;" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Menu</th>
                                <th>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1.</td>
                                <td>Amala</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>2.</td>
                                <td>Garri</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>3.</td>
                                <td>Poundo Yam</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>4.</td>
                                <td>Pounded Yam</td>
                                <td>₦3,500</td>
                            </tr>
                            <tr>
                                <td>5.</td>
                                <td>Semolina</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>6.</td>
                                <td>Tuwon Rice</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>7.</td>
                                <td>Wheat</td>
                                <td>₦1,500</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

    <div class="section padding-top z-bigger">
        <div class="container">
            <div class="row justify-content-center padding-bottom-smaller">
                <div class="col-md-8">
                    <div class="subtitle with-line text-center mb-4">Place your order Now</div>
                    <h3 class="text-center padding-bottom-small">Breakfast</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-12 col-lg-12">
                    <table id="breakfast-table" style="width: 100%;" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Menu</th>
                                <th>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1.</td>
                                <td>Chips & Omelette</td>
                                <td>₦4,500</td>
                            </tr>
                            <tr>
                                <td>2.</td>
                                <td>Chips & Egg Sauce</td>
                                <td>₦5,000</td>
                            </tr>
                            <tr>
                                <td>3.</td>
                                <td>Chips & Kidney Sauce</td>
                                <td>₦6,000</td>
                            </tr>
                            <tr>
                                <td>4.</td>
                                <td>Chips & Chicken Sauce</td>
                                <td>₦6,500</td>
                            </tr>
                            <tr>
                                <td>5.</td>
                                <td>Chips & Fish Sauce</td>
                                <td>₦6,500</td>
                            </tr>
                            <tr>
                                <td>6.</td>
                                <td>Fried Yam & Egg Sauce</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>7.</td>
                                <td>Toasted Bread & Sardine</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>8.</td>
                                <td>Toasted Bread & Omelette</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>9.</td>
                                <td>Bread & Omelette</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>10.</td>
                                <td>Sandwich</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>11.</td>
                                <td>Chip Sandwich</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>12.</td>
                                <td>Plantain</td>
                                <td>₦800</td>
                            </tr>
                            <tr>
                                <td>13.</td>
                                <td>Jollof Spaghetti</td>
                                <td>₦1,200</td>
                            </tr>
                            <tr>
                                <td>14.</td>
                                <td>White Spaghetti & Stew</td>
                                <td>₦1,800</td>
                            </tr>
                            <tr>
                                <td>15.</td>
                                <td>Indomie</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>16.</td>
                                <td>Spanish Omelette</td>
                                <td>₦1,000</td>
                            </tr>
                            <tr>
                                <td>17.</td>
                                <td>White Oat</td>
                                <td>₦500</td>
                            </tr>
                            <tr>
                                <td>18.</td>
                                <td>Jollof Rice</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>19.</td>
                                <td>Fried Rice</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>20.</td>
                                <td>White Rice</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>21.</td>
                                <td>Jollof Cuscus</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>22.</td>
                                <td>White Cuscus</td>
                                <td>₦1,200</td>
                            </tr>
                            <tr>
                                <td>23.</td>
                                <td>Club Sandwich</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>24.</td>
                                <td>Masa Small</td>
                                <td>₦200</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

    <div class="section padding-top-bottom over-hide background-grey">
        <div class="container">
            <div class="row justify-content-center padding-bottom-smaller">
                <div class="col-md-8">
                    <div class="subtitle with-line text-center mb-4">Place your order Now</div>
                    <h3 class="text-center padding-bottom-small">Beverages</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-12 col-lg-12">
                    <table id="beverages-table" style="width: 100%;" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Menu</th>
                                <th>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1.</td>
                                <td>Nescafe</td>
                                <td>₦200</td>
                            </tr>
                            <tr>
                                <td>2.</td>
                                <td>Lipton</td>
                                <td>₦200</td>
                            </tr>
                            <tr>
                                <td>3.</td>
                                <td>Coffee & Cream</td>
                                <td>₦1,000</td>
                            </tr>
                            <tr>
                                <td>4.</td>
                                <td>Black Coffee</td>
                                <td>₦1,000</td>
                            </tr>
                            <tr>
                                <td>5.</td>
                                <td>Black Tea</td>
                                <td>₦1,000</td>
                            </tr>
                            <tr>
                                <td>6.</td>
                                <td>Tin Milk</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>7.</td>
                                <td>Sachet Milk</td>
                                <td>₦300</td>
                            </tr>
                            <tr>
                                <td>8.</td>
                                <td>Milo</td>
                                <td>₦300</td>
                            </tr>
                            <tr>
                                <td>9.</td>
                                <td>Bournvita</td>
                                <td>₦300</td>
                            </tr>
                            <tr>
                                <td>10.</td>
                                <td>Hollandia</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>11.</td>
                                <td>Chi Active</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>12.</td>
                                <td>Chi Exotic</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>13.</td>
                                <td>Big Five Alive</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>14.</td>
                                <td>Maltina</td>
                                <td>₦1,000</td>
                            </tr>
                            <tr>
                                <td>15.</td>
                                <td>Fearless</td>
                                <td>₦1,000</td>
                            </tr>
                            <tr>
                                <td>16.</td>
                                <td>Coke</td>
                                <td>₦800</td>
                            </tr>
                            <tr>
                                <td>17.</td>
                                <td>Small Water</td>
                                <td>₦500</td>
                            </tr>
                            <tr>
                                <td>18.</td>
                                <td>Big Water</td>
                                <td>₦1,000</td>
                            </tr>
                            <tr>
                                <td>19.</td>
                                <td>Fizzy</td>
                                <td>₦600</td>
                            </tr>
                            <tr>
                                <td>20.</td>
                                <td>Vita Milk</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>21.</td>
                                <td>Kunu</td>
                                <td>₦800</td>
                            </tr>
                            <tr>
                                <td>22.</td>
                                <td>Fresh Yo B/S</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>23.</td>
                                <td>Fresh Yo PET</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>24.</td>
                                <td>Dano Milk</td>
                                <td>₦3,500</td>
                            </tr>
                            <tr>
                                <td>25.</td>
                                <td>Grand Malt</td>
                                <td>₦500</td>
                            </tr>
                            <tr>
                                <td>26.</td>
                                <td>4th Street</td>
                                <td>₦12,000</td>
                            </tr>
                            <tr>
                                <td>27.</td>
                                <td>Chamdor</td>
                                <td>₦10,000</td>
                            </tr>
                            <tr>
                                <td>28.</td>
                                <td>Eva Wine</td>
                                <td>₦12,000</td>
                            </tr>
                            <tr>
                                <td>29.</td>
                                <td>Pure Heaven</td>
                                <td>₦12,000</td>
                            </tr>
                            <tr>
                                <td>30.</td>
                                <td>J & W</td>
                                <td>₦15,000</td>
                            </tr>
                            <tr>
                                <td>31.</td>
                                <td>Fayrouz</td>
                                <td>₦1,000</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

    <div class="section padding-top z-bigger">
        <div class="container">
            <div class="row justify-content-center padding-bottom-smaller">
                <div class="col-md-8">
                    <div class="subtitle with-line text-center mb-4">Place your order Now</div>
                    <h3 class="text-center padding-bottom-small">Sauce</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-12 col-lg-12">
                    <table id="sauce-table" style="width: 100%;" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Menu</th>
                                <th>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1.</td>
                                <td>Chicken Sauce</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>2.</td>
                                <td>Fish Sauce</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>3.</td>
                                <td>Kidney Sauce</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>4.</td>
                                <td>Egg Sauce</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>5.</td>
                                <td>Vegetable Sauce</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>6.</td>
                                <td>Cream Soup</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>7.</td>
                                <td>Tomato Sauce</td>
                                <td>₦500</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

    <div class="section padding-top-bottom over-hide background-grey">
        <div class="container">
            <div class="row justify-content-center padding-bottom-smaller">
                <div class="col-md-8">
                    <div class="subtitle with-line text-center mb-4">Place your order Now</div>
                    <h3 class="text-center padding-bottom-small">Starters</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-12 col-lg-12">
                    <table id="starters-table" style="width: 100%;" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Menu</th>
                                <th>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1.</td>
                                <td>Chicken Pepper Soup</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>2.</td>
                                <td>Cowtail Pepper Soup</td>
                                <td>₦6,000</td>
                            </tr>
                            <tr>
                                <td>3.</td>
                                <td>Fish Pepper Soup</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>4.</td>
                                <td>Goat Meat Pepper Soup</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>5.</td>
                                <td>Goat Meat Pepper Soup B/S</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>6.</td>
                                <td>Kayanciki Pepper Soup</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>7.</td>
                                <td>Salad</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>8.</td>
                                <td>Green Salad</td>
                                <td>₦4,500</td>
                            </tr>
                            <tr>
                                <td>9.</td>
                                <td>Vegetable Salad</td>
                                <td>₦4,500</td>
                            </tr>
                            <tr>
                                <td>10.</td>
                                <td>Fruit Salad</td>
                                <td>₦4,000</td>
                            </tr>
                            <tr>
                                <td>11.</td>
                                <td>Cole Slaw</td>
                                <td>₦700</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

    <div class="section padding-top z-bigger">
        <div class="container">
            <div class="row justify-content-center padding-bottom-smaller">
                <div class="col-md-8">
                    <div class="subtitle with-line text-center mb-4">Place your order Now</div>
                    <h3 class="text-center padding-bottom-small">Others</h3>
                </div>
                <div class="section clearfix"></div>
                <div class="col-md-12 col-lg-12">
                    <table id="others-table" style="width: 100%;" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Menu</th>
                                <th>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1.</td>
                                <td>Boiled Yam</td>
                                <td>₦2,500</td>
                            </tr>
                            <tr>
                                <td>2.</td>
                                <td>Yam Portage</td>
                                <td>₦3,000</td>
                            </tr>
                            <tr>
                                <td>3.</td>
                                <td>White Spaghetti</td>
                                <td>₦800</td>
                            </tr>
                            <tr>
                                <td>4.</td>
                                <td>Bread</td>
                                <td>₦500</td>
                            </tr>
                            <tr>
                                <td>5.</td>
                                <td>Toasted Bread</td>
                                <td>₦700</td>
                            </tr>
                            <tr>
                                <td>6.</td>
                                <td>Sardine</td>
                                <td>₦1,500</td>
                            </tr>
                            <tr>
                                <td>7.</td>
                                <td>Boiled Egg</td>
                                <td>₦1,000</td>
                            </tr>
                            <tr>
                                <td>8.</td>
                                <td>Scrambled Eggs</td>
                                <td>₦1,100</td>
                            </tr>
                            <tr>
                                <td>9.</td>
                                <td>Aloo Parata</td>
                                <td>₦6,000</td>
                            </tr>
                            <tr>
                                <td>10.</td>
                                <td>Biscuit Bone</td>
                                <td>₦2,000</td>
                            </tr>
                            <tr>
                                <td>11.</td>
                                <td>Trade Charge</td>
                                <td>₦100</td>
                            </tr>
                            <tr>
                                <td>12.</td>
                                <td>Take Away</td>
                                <td>₦300</td>
                            </tr>
                            <tr>
                                <td>13.</td>
                                <td>Take Away Plate</td>
                                <td>₦200</td>
                            </tr>
                            <tr>
                                <td>14.</td>
                                <td>Pinnacle Bag</td>
                                <td>₦600</td>
                            </tr>
                            <tr>
                                <td>15.</td>
                                <td>Tray Charge</td>
                                <td>₦500</td>
                            </tr>
                            <tr>
                                <td>16.</td>
                                <td>Microwave S/S</td>
                                <td>₦300</td>
                            </tr>
                            <tr>
                                <td>17.</td>
                                <td>Microwave B/S</td>
                                <td>₦500</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>


    </div>

    </div>
    </div>
@endsection
